...
ここで注意しなければならないのは、コンテナ内で作成した実行可能ファイルはコンテナの環境で実行できるように作られているため、**`docker run`コマンドを用いて同じ環境を再現しなければ実行することができない**ということです。
...
今回は、DockerでGo言語を導入してみました。Dockerを利用すれば、環境を手軽に試すことができます。この記事で紹介した方法以外にもGo言語を扱う方法は存在しているので、環境や用途に合わせて自身に合ったよう方法を選択することが大事です。
