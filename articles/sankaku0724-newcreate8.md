...
どうやら、**環境変数として保存しておけば、すべてのプログラムでその値を使うことができる**ようです。（普段、何気なく通してるPATHとかもその一種らしい。）
...
セキュリティ上の理由から、秘密情報を含む場合には.envファイルを適切に保護する必要があるでしょう。（Gitを用いる際に、.gitignoreに追加するなど！）
...
つまり、大まかな役割は先ほどの.envと同様というように見えますが、**JSONファイルは環境変数以外にも、汎用的なデータの保存に使用され、データの構造化や交換に適している**という特徴があるようです。

## JSON使ってみた

先ほどと同様にして、LINE.pyに記載していたLINE Notifyのアクセストークンを環境変数としてJSONファイルに設定してみます。
...
```
Pythonの辞書に似た見た目の構造でデータを格納することができました。（あくまでも見た目が似ているだけで、全くの別物のようですが。）
:::message
...
今回は環境変数を格納しましたが、その他のデータを外部参照する際にもJSONは役立つでしょう。

## まとめ

今回、実際に環境変数を利用した中で、以下の点について理解を深めることができました。
...
4. **機密情報を環境変数として外部参照することで、セキュリティを向上させることができる。**

今回、私はPythonで環境変数を利用しましたが、他のプログラミング言語でも、.envやJSONファイル、その他のファイルを用いた環境変数の管理は可能です。
そのため、使用する言語や環境に合ったものを選択して利用する必要があります。
