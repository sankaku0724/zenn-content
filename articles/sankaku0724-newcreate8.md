...
どうやら、**環境変数として保存しておけば、すべてのプログラムでその値を使うことができる**ようです。（PATHとかもその一種）
...
まず、プロジェクトのルートディレクトリに.envというファイルを作成し、その中に環境変数を記述します。その後、プログラム内でpython-dotenvを使用して.envファイルから環境変数を取得します。
...
つまり、大まかな役割は先ほどの.envと同様ということですが、**JSONファイルは環境変数以外にも、汎用的なデータの保存に使用され、データの構造化や交換に適している**という特徴があると言えるでしょう。
