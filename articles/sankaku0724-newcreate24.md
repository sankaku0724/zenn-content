...
[**GitHub**](https://github.co.jp/)は、プログラムのソースコードをオンラインで共有・管理するためのサービスであり、手元のファイルやディレクトリをGithubにプッシュ（ローカルリポジトリの変更をリモートリポジトリに反映）することで保存することができます。また、これは、ソースコードのバージョン管理システムであるGitを基にしたものになっています。
